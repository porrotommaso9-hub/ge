...
document.querySelectorAll('a[href]').forEach(a => {
  ...
  // Escludi: link esterni, anchor, tel, mailto, wa, e LANG TOGGLE
  if(!href || href.startsWith('http') || href.startsWith('#')
     || href.startsWith('tel:') || href.startsWith('mailto:')
     || href.startsWith('https://wa.me')
     || a.classList.contains('lang-btn')
     || a.classList.contains('mmenu__lang-btn')) return;
...
// Lang toggle — bubble phase, no stopPropagation
document.addEventListener('click', function(e) {
  var btn = e.target.closest('.lang-btn, .mmenu__lang-btn');
...
// Enter animation on load — curtain esce dalla pagina nuova
...
/* ══ FORM HANDLER — Formspree async ══
   Crea account su formspree.io e sostituisci gli ID qui sotto */
(function(){
  var ENDPOINTS = {
    'events':      'https://formspree.io/f/EVENTS_FORM_ID',
    'franchising': 'https://formspree.io/f/FRANC_FORM_ID'
  };
  async function submit(form, key) {
    var btn = form.querySelector('.lx-submit');
    var ok  = form.querySelector('.lx-msg.success');
    var err = form.querySelector('.lx-msg.error');
    if (btn) { btn.disabled = true; btn.classList.add('loading'); }
    if (ok)  ok.style.display  = 'none';
    if (err) err.style.display = 'none';
    try {
      var res = await fetch(ENDPOINTS[key], {
  ...
  document.querySelectorAll('[data-form="events"]').forEach(function(f){
    f.addEventListener('submit', function(e){ e.preventDefault(); submit(f,'events'); });
  });
  document.querySelectorAll('[data-form="franchising"]').forEach(function(f){
    f.addEventListener('submit', function(e){ e.preventDefault(); submit(f,'franchising'); });
  });
})();
...
(function(){
  ...
  if(!slider) return;
  var slides = slider.querySelectorAll('.food-slide');
  var dots   = slider.querySelectorAll('.food-dot');
  var cur = 0, timer;
  function goTo(n){
    slides[cur].classList.remove('food-slide--active');
    dots[cur].classList.remove('food-dot--active');
    cur = (n + slides.length) % slides.length;
    slides[cur].classList.add('food-slide--active');
    dots[cur].classList.add('food-dot--active');
  }
...
/* ══ LANG TOGGLE — registra scelta utente ══ */
(function(){
  document.querySelectorAll('a[hreflang="en"]').forEach(function(a){
    a.addEventListener('click', function(){ try{localStorage.setItem('dg_lang','en');}catch(e){} });
  });
  document.querySelectorAll('a[hreflang="it"]').forEach(function(a){
    a.addEventListener('click', function(){ try{localStorage.setItem('dg_lang','it');}catch(e){} });
  });
})();
